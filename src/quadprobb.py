...
from .qphelpers import checkinputs, getsol, fixedAb, isfeasible
from .qpreformulation import standardform
from .qpcplexopt import quadprog
import numpy as np
...
def quadprogbb(H, f, A=np.empty([0, 0]), b=np.array([]), Aeq=np.empty([0, 0]),
    ...
    # -------------------------
    LBLB = np.array([-np.inf])
    FxFx, FzFz = list(), list()
    FxFx.append(Fx)
    FzFz.append(Fz)
    SS = np.zeros(((1 + bign) ** 2, 1))
    SIGSIG = np.array([-1])  # Signal that we want default sig in aug Lag algorithm
    ...
    if options['use_quadprog']:
        ...
        xx = np.array(solution["x"])
        gUB = solution["primal objective"]
    else:
        xx = np.array([])
        gUB = np.inf
    ...
    # ------------------------------------------
    #
    # Store fixed components index to prevent these variables's bounds from changing
    Fx0 = Fx
    Fz0 = Fz
    ...
    while length(LBLB) > 0:
        ...
        #
        if options["verbosity"]:
            print('STATUS 1: (gUB, gLB, gap) = (%.8e, %.8e, %.3f)' %
                  (gUB + cons, min(LBLB) + cons, 100 * (gUB - min(LBLB)) / max([1, abs(gUB + cons)])), flush=True)
            print('STATUS 2: (created, solved, pruned, infeas, left) = (%d, %d, %d, %d, %d)' %
                  (nodes_created, nodes_solved, nodes_pruned, nodes_infeasible, length(LBLB)), flush=True)
            print('STATUS 3: solved = fully + fathomed + poorly : %d = %d + %d + %d' %
                  (nodes_solved, nodes_solved_fully, nodes_solved_fathomed, nodes_solved_poorly), flush=True)
            # print('STATUS 4: time = %d\n', toc)
            print('====================== Node %d ===========================\n' % (nodes_solved + 1), flush=True)
        ...
        I = np.argsort(LBLB)[::-1]
        FxFx = list(itemgetter(*I)(FxFx))
        FzFz = list(itemgetter(*I)(FzFz))
        SS = SS[:, I]
        SIGSIG = SIGSIG[I]
        ...
        LB = LBLB[-1]
        Fx = FxFx[-1] if len(FxFx) > 0 else []
        Fx = Fx[1] if len(Fx) > 0 else []
        Fz = FzFz[-1] if len(FzFz) > 0 else []
        Fz = Fz[1] if len(Fz) > 0 else []
        S = SS[:, -1].reshape(1 + bign, 1 + bign)
        SIG = SIGSIG[-1]
        # Signal that we want default sig in aug Lag algorithm
        if SIG < 0.0:
            SIG = np.array([])
        #
        # ---------------------------------
